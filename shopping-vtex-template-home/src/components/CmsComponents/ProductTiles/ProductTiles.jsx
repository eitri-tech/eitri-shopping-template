			...
			<View className='overflow-x-auto flex px-4 gap-2'>
				{shelves?.map(shelf => (
					<View
						...
						className={`py-1 px-2 border min-w-fit rounded-full ${
							shelf.title === currentShelf.title ? 'border-blue-300' : 'border-neutral-300'
						}`}>
						<Text
							className={`${shelf.title === currentShelf.title ? 'text-blue-300' : 'text-neutral-300'}`}>
							{shelf.title}
						</Text>
					</View>
				))}
